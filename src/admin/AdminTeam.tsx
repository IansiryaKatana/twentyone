...
type Row = Tables<"team_members">;

const TEAM_FILTERS: AdminFilterDef[] = [
  {
    key: "published",
    label: "Published",
    options: [
...
export function AdminTeam() {
  ...
  const filterState = useAdminFilters(TEAM_FILTERS);

  const filteredRows = useMemo(() => {
    return rows.filter((row) => {
      if (!matchesQuery(`${row.name} ${row.title}`, filterState.query)) return false;
  ...
  const save = async () => {
    ...
    if (!draft.name.trim() || !draft.title.trim()) {
      setSaveErr("Name and title are required.");
      return;
    }
  ...
  const confirmDelete = async () => {
    ...
    const sb = getSupabase();
    if (!sb) return;
    const { error } = await sb.from("team_members").delete().eq("id", deleteId);
    if (error) setErr(error.message);
    else {
      setDeleteId(null);
      await refresh();
  ...
  const confirmBulkDelete = async () => {
    ...
    if (!sb) return;
    setBulkBusy(true);
    const { error } = await sb.from("team_members").delete().in("id", selection.selectedList);
    setBulkBusy(false);
    if (error) {
      setErr(error.message);
      return;
  ...
  return (
    <div>
      <AdminPageHeading
        title="Team"
        description="Portrait cards for the About page. Drag rows to set display order. LinkedIn on every card when set; Instagram optional. Upload photos when ready."
        actions={
          canMutate ? (
      ...
      <div className={adminTableWrap}>
        <AdminFilters
          ...
          queryPlaceholder="Search name or title…"
          filterDefs={TEAM_FILTERS}
            ...
            <AdminSortableBody itemIds={reorder.itemIds}>
              {pagination.pageRows.map((row) => (
                ...
                >
                  ...
                  <td className={adminTableCell}>
                    {row.image ? (
                      <img src={row.image} alt="" className="h-12 w-10 rounded object-cover" />
                    ) : (
                      <span className="text-xs text-[var(--admin-muted)]">No photo</span>
                    )}
                  </td>
                  ...
                  <td className={adminTableCell}>{row.published ? "Published" : "Draft"}</td>
                  <td className={adminTableCell}>
      ...
      <AdminModal
        ...
        title={draft?.name ? `Edit ${draft.name}` : "Add team member"}
        onSave={() => void save()}
      ...
      >
        {draft ? (
          <div className="space-y-4">
            ...
            <div>
              <label className={adminLabel}>Title (on card)</label>
              <input
                className={adminInput}
                value={draft.title}
            ...
            <div>
              <label className={adminLabel}>Instagram URL (optional)</label>
              <input
                className={adminInput}
      ...
      <AdminModal
        ...
        description={`Permanently delete ${selection.selectedCount} member${selection.selectedCount === 1 ? "" : "s"}?`}
