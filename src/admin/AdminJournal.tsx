...
type Row = Tables<"journal_posts">;



const JOURNAL_FILTERS: AdminFilterDef[] = [
...
export function AdminJournal() {
  ...
  const filterState = useAdminFilters(JOURNAL_FILTERS);
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (!matchesQuery(`${row.title} ${row.slug} ${row.tag}`, filterState.query)) {
  ...
  const save = async () => {
    ...
    const { error } = await sb.from("journal_posts").upsert(payload, { onConflict: "id" });
  ...
  const confirmDelete = async () => {
    ...
    const { error } = await sb.from("journal_posts").delete().eq("id", deleteId);
  ...
  const confirmBulkDelete = async () => {
    ...
    const { error } = await sb.from("journal_posts").delete().in("id", selection.selectedList);
  ...
  return (
    ...
    <div>
      ...
      <AdminPageHeading
        ...
        description="Long-form articles and studio notes. Drag rows to set display order."
      ...
      <div className={adminTableWrap}>
        ...
        <AdminFilters
          ...
          queryPlaceholder="Search title, slug, tag…"

          filterDefs={JOURNAL_FILTERS}
            ...
            >
              ...
              {pagination.pageRows.map((row) => (
                ...
                >
                  ...
                  <td className={adminTableCell}>{row.date}</td>
              ...
              {pagination.pageRows.length === 0 ? (
                ...
                <tr>
                  ...
                  <td className={adminTableCell} colSpan={canMutate ? 6 : 4}>
      ...
      <AdminModal
        ...
        title={draft && draft.created_at === draft.updated_at ? "New post" : "Edit post"}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <label className={adminLabel}>Tag</label>
            ...
            <div>
              ...
              <label className={adminLabel}>Rich body (HTML)</label>
      ...
      <AdminModal
        ...
        description={`Permanently delete ${selection.selectedCount} post${selection.selectedCount === 1 ? "" : "s"}? This cannot be undone.`}
      ...
      <EntityDetailSheet
        ...
        title={viewRow?.title ?? "Post"}

      >

        {viewRow ? (

          <p className="text-sm text-[var(--admin-muted)]">{viewRow.excerpt}</p>
