...
import type { AwardStatus } from "@/data/content";
...
const AWARD_FILTERS: AdminFilterDef[] = [
  {
    key: "status",
    label: "Status",
...
export function AdminAwards() {
  ...
  const filterState = useAdminFilters(AWARD_FILTERS);
  ...
  const save = async () => {
    ...
    if (!draft.title.trim()) {
      setSaveErr("Award title is required.");
      return;
    }
    ...
    setSaving(true);
    setSaveErr(null);
    const { error } = await sb.from("awards").upsert(
      {
        ...draft,
  ...
  const confirmDelete = async () => {
    ...
    const sb = getSupabase();
    if (!sb) return;
    const { error } = await sb.from("awards").delete().eq("id", deleteId);
    if (error) setErr(error.message);
    else {
      setDeleteId(null);
      await refresh();
  ...
  const confirmBulkDelete = async () => {
    ...
    if (!sb) return;
    setBulkBusy(true);
    const { error } = await sb.from("awards").delete().in("id", selection.selectedList);
    setBulkBusy(false);
    if (error) {
      setErr(error.message);
      return;
  ...
  return (
    <div>
      <AdminPageHeading
        title="Awards"
        description="Recognition list on the About page. Drag rows to set display order. Colour-coded by status."
        actions={
          canMutate ? (
      ...
      <div className={adminTableWrap}>
        <AdminFilters
          ...
          queryPlaceholder="Search awards…"
          filterDefs={AWARD_FILTERS}
      ...
      <AdminModal
        ...
        title={draft?.title ? "Edit award" : "Add award"}
        onSave={() => void save()}
      ...
      <AdminModal
        ...
        description={`Permanently delete ${selection.selectedCount} award${selection.selectedCount === 1 ? "" : "s"}?`}
