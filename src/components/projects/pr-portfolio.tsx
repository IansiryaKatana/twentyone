...
export function PrProjectGrid({
  ...
  const visibleProjects = paginated
    ...
    : filtered;
  const hasMore = paginated && visibleCount < filtered.length;
  const isExpanded =
    paginated && visibleCount > PAGE_SIZE && filtered.length > PAGE_SIZE;
    ...
    <>
      ...
      {filtered.length === 0 ? (
        <p className="mt-12 text-sm text-white/50">
          No projects match {showFilters ? "these filters" : "this selection"}{" "}
          yet.
          {showFilters ? (
            <>
              {" "}
